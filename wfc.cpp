...
// BuildingBlock(string name, vector<int> bitmap, bool* sides, float weight, float density)
// UP DOWN LEFT RIGHT
// EVERY TILE IS 3x3 bitmap
...
//Getters
int WFC::GetNumThreads(){
  return threads; 
}

vector<vector<WFCBlock>> WFC::GetTilemap(){
  return tilemap; 
}
...
vector<WFC::Neighbor> WFC::get_neighbors(location loc, int height, int width) {
  ...
  if (loc.x != 0)
    neighborTup.push_back(Neighbor(DIRECTION::RIGHT, location(loc.x - 1, loc.y)));
  if (loc.y != 0)
  ...
  if (loc.y < width - 1)
    neighborTup.push_back(
        Neighbor(DIRECTION::UP, location(loc.x, loc.y + 1)));
  return neighborTup;
}


// Finds all the tiles with the lowest # of building blocks left to collapse and returns one at random
...
// Helper function used by propagate to build a list of locations that need updating
...
#pragma omp parallel for num_threads(threads)
    for (int col = 0; col < width; col++) {
      if (tilemap[row][col].needsUpdate)
        locs.push_back(location(row, col));
    }
  }
  return locs;
}
...
bool WFC::add_constraint(Neighbor neighbor,
  ...
  if ((int)tilemap[neighbor.loc.y][neighbor.loc.x].superPositions.size() != 1){
    ...
    for (int i = 0; i < (int)tilemap[neighbor.loc.y][neighbor.loc.x].superPositions.size(); i++) {
      auto searchVal = tilemap[neighbor.loc.y][neighbor.loc.x].superPositions[i].connections[incDirection];
...
// Meat & potatoes; Propagates constraints out to all needed tiles based on the most recently collapsed tile
void WFC::Propogate(location next_location) {
  ...
  // Loop to continue propagating until there are no more tiles that have been constrained
...
#pragma omp parallel for num_threads(threads)
      for (auto& neighbor : neighbors) {
        ...
        bool wasUpdated = add_constraint(neighbor, tilemap[location.y][location.x].superPositions);
        
        // If any constraints were added, mark it as needing to update so we propagate from this location next
...
// ONLY WORKS WITH BUILDING BLOCKS THAT HAVE NAMES LIKE WHAT IS PASSED IN ITERATESPECIFIC
void WFC::MakeMaze(){
  int in = rand() % width;
  int out = rand() % width; 
